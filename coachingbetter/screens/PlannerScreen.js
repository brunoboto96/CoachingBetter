...
import predictions from '../components/predictions.service';
...
export default class HomeScreen extends React.Component {
  ...
  newSession = () => {
    category.getCategoryByName(this.state.category_name).then((data) => {
      ...
      session.newSession(this.state.session_title, this.state.session_description, category.selectedCategory._id, this.state.programsData[0]._id).then((data) => {
        ...
        console.log("sessionid1: ", session.selectedSession._id)
        this.newModule()
      })
    })
  }
  ...
  newModule = () => {
    modules.newModules(this.state.module_explanation, session.selectedSession._id).then((data) => {
      console.log("sessionid: ", session.selectedSession._id)
  ...
  predictions = () => {
    predictions.getCategoryId(this.state.category_name).then((data) => {
      predictions.getSessionByCategoryFk(predictions.selectedCategory._id).then(() => {
        let sessions = predictions.selectedSessions.session
        let modules = []
        let activities = []
        let feedbacks = []

        for (let i = 0; i < sessions.length; i++) {
          predictions.getModuleBySessionFk(predictions.selectedSessions.session[i]._id).then(() => {
            predictions.getActivityByModuleFk(predictions.selectedModules.module[0]._id).then(() => {
              activities.push(predictions.selectedActivities.activity.name)
              ...
              if (i == sessions.length - 1) {
                ...
                console.log("sessionssz: ", sessions[0].programFk)
                for (let x = 0; x < sessions.length; x++) {
                  predictions.getProgramBySessionFk(sessions[x].programFk).then(() => {
                    predictions.getFeedbackByProgramFk(predictions.selectedPrograms.program._id).then(() => {
                      feedbacks.push(predictions.selectedFeedbacks.feedback)
                      console.log("feedbacks: ", feedbacks)
                      console.log("round: ", x, " ", i)
                      if (x == sessions.length - 1) {

                        for (let p = 0; p < feedbacks.length; p++) {
                          let normalp = 0
                          for (let p2 = 0; p2 < feedbacks[p].length; p2++) {
                            normalp += feedbacks[p][p2].rating
                          }
                          console.log("Predictions: \nProbability: ", normalp * 10 / feedbacks[p].length,
                          "\Your probability: C is the best Option",
                          "\nMedian: ", 56 ,
                          "\Your probability: A is the best Option",
                          "\nTeachers: ", 65,
                          "\Your probability: A is the best Option",)
                        }
                      }
...
console.log("modulesid: ", modules.then(modules[0][0]))
 
for (let i = 0; i < sessions.length; i++) {
  predictions.getActivityByModuleFk(modules[i]._id).then(() => {
    activities.push(predictions.selectedActivities.activity)
  })
}
